...
knitr::include_graphics("figs/Original data figure.png",dpi = NA)
```
...
The ``simdata.rda`` have already been resampled from 32k to 2k resolution to reduce computation time. Next, we resample the background surface (i.e., template) to the same resolution, which will allow us to plot the loadings on the cortical surface. This step uses **ciftiTools** [@pham2021ciftitools] and connectome workbench [@marcus2011informatics]. To run this code, one needs to install connectome workbench, as described in (https://github.com/mandymejia/ciftiTools). 
...
## the template cifti file is built in the package, and resample to 2k resolution.
...
```{r truex,echo=FALSE,out.width = "100%",fig.cap="True joint loadings in SJX"}
knitr::include_graphics("figs/Truth_CompX.png",dpi = NA)
```
...
out_true1 = plotNetwork_change(Sytrue[1,], title=expression("True S"["Jx"]*", 1"),
                               qmin=0.005, qmax=0.995, path = path) 
out_true2 = plotNetwork_change(Sytrue[2,], title=expression("True S"["Jx"]*", 2"),
                               qmin=0.005, qmax=0.995, path = path) 
...
```{r estx2,echo=FALSE,out.width = "100%",fig.cap="Estimated joint loadings in SJX in example 2."}
knitr::include_graphics("figs/Esti_CompX.png",dpi = NA)
```
...
library(cowplot)
path = "C:/Liangkang Wang/Emory/Research/sing/singR/extdata/new_mmp.csv"
...
```{r esty2,echo=FALSE,out.width = "100%",fig.cap="Estimated joint loadings in SJY in example 2."}
knitr::include_graphics("figs/Estexp2.png",dpi = NA)
```
